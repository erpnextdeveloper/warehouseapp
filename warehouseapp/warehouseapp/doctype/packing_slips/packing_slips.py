...
class PackingSlips(Document):
	def autoname(self):
		...
		w_prefix=frappe.db.get_value("Warehouse",self.warehouse,"warehouse_code")
		year_prefix=frappe.db.get_value("Year",mydate.year,"prefix")
		month_prefix=frappe.db.get_value("Month",date.strftime("%B"),"prefix")
		idx = self.package_no

		self.name = 'MB'+str(w_prefix)+'-'+str(self.id_generator()) + ('%.3i' % idx)

	def id_generator(self):
		num=''.join(random.choice('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ') for _ in range(3))
